...
%% compute fft of the 2 sources
% retrieve single trial sources
tc1 = cellfun(@(x) source.avg.filter{k(1)} * x,data.trial,'uniformoutput',0);% time course of single trials
tc2 = cellfun(@(x) source.avg.filter{k(2)} * x,data.trial,'uniformoutput',0);

% I'll take the 1st pc for each source
w1 = pca(cat(2,tc1{:})');
w2 = pca(cat(2,tc2{:})');
tc11 = cellfun(@(x)w1(1,:) * x,tc1,'uniformoutput',0);
tc21 = cellfun(@(x)w2(1,:) * x,tc2,'uniformoutput',0);
source4fft = [];
source4fft.trial = cellfun(@vertcat,tc11,tc21,'uniformoutput',0);
source4fft.time = repmat({source.time},size(source4fft.trial));
source4fft.label = {'source1' 'source2'};
source4fft.dimord = 'chan_time';
...
cfg.foi    = 10;
cfg.taper  = 'hanning';
freq = ft_freqanalysis(cfg,source4fft);

figure;hist(angle(freq.fourierspctrm))
xlim([0 pi/2])
